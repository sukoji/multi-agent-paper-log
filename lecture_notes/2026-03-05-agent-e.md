# Agent-E: Hierarchical planning for web agents

> **see paper** · 읽은 날짜: 2026-03-05  
> 분류: 웹 · GUI · computer-use 에이전트

### 링크
- [Original Paper / Resource](https://scholar.google.com/scholar?q=Agent-E:+Hierarchical+planning+for+web+agents)
- [Summary Note](./2026-03-05-agent-e.md)

---

## 한 줄 요약

Agent-E: Hierarchical planning for web agents — observation space(DOM vs screenshot)와 action granularity 체크.

## 문제 정의

real web complexity + grounding error.

## 방법 · 핵심 아이디어

planner + browser controller / multimodal grounding.

## 실험 · 결과

success rate human gap still large.

## TIL — 내가 가져간 점

eval에 failure taxonomy(grounding vs planning) 넣기.

---

*개인 공부 노트입니다. [Deep-Learning-Paper-Review-and-Practice](https://github.com/SukoJim/Deep-Learning-Paper-Review-and-Practice) 형식을 참고했습니다.*
