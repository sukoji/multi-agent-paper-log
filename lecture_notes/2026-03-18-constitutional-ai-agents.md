# Constitutional AI applied to agent loops

> **see paper** · 읽은 날짜: 2026-03-18  
> 분류: 안전 · 정렬

### 링크
- [Original Paper / Resource](https://scholar.google.com/scholar?q=Constitutional+AI+applied+to+agent+loops)
- [Summary Note](./2026-03-18-constitutional-ai-agents.md)

---

## 한 줄 요약

Constitutional AI applied to agent loops — misuse surface와 mitigation이 eval에 포함됐는지.

## 문제 정의

capable agent = larger attack surface.

## 방법 · 핵심 아이디어

constitution / filter / sandbox / human approval.

## 실험 · 결과

harm rate or refusal tradeoff.

## TIL — 내가 가져간 점

self-evolving agent와 결합 시 risk 증폭.

---

*개인 공부 노트입니다. [Deep-Learning-Paper-Review-and-Practice](https://github.com/SukoJim/Deep-Learning-Paper-Review-and-Practice) 형식을 참고했습니다.*
