# Corrective RAG / agentic RAG for tool loops

> **see paper** · 읽은 날짜: 2026-04-19  
> 분류: 메모리 · RAG

### 링크
- [Original Paper / Resource](https://scholar.google.com/scholar?q=Corrective+RAG+/+agentic+RAG+for+tool+loops)
- [Summary Note](./2026-04-19-rag-agents.md)

---

## 한 줄 요약

Corrective RAG / agentic RAG for tool loops — write/read trigger와 context budget tradeoff.

## 문제 정의

long horizon에서 fact drift.

## 방법 · 핵심 아이디어

episodic + semantic memory tier or RAG gate.

## 실험 · 결과

multi-hop QA / long dialog metric.

## TIL — 내가 가져간 점

MemGPT-style paging 고려.

---

*개인 공부 노트입니다. [Deep-Learning-Paper-Review-and-Practice](https://github.com/SukoJim/Deep-Learning-Paper-Review-and-Practice) 형식을 참고했습니다.*
